...
/** Approximate USD pricing per 1M tokens (estimates). */
...
export function getRate(
  ...
  const table = pricing[provider] ?? pricing.openai ?? { default: { input: 0.5, output: 1.5 } };
  if (table[model]) return table[model]!;
  const fuzzy = Object.entries(table)
    .filter(([k]) => k !== "default" && model.includes(k))
    .sort((a, b) => b[0].length - a[0].length)[0];
  return fuzzy?.[1] ?? table.default ?? { input: 0.5, output: 1.5 };
}
